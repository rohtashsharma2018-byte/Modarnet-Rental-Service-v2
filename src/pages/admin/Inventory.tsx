...
export default function Inventory() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.error(`Failed to delete: ${e.message || 'Check permissions'}`);
          ...
          <div className="p-4 bg-slate-50 border-b border-slate-200 space-y-4">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1">Total Assets (₹)</label>
                  <input type="number" step="0.01" {...register("price")} className="w-full rounded border border-slate-300 px-3 py-1.5 text-sm" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1">Offer Price Per Item (₹)</label>
                  <input type="number" step="0.01" {...register("offerPricePerItem")} className="w-full rounded border border-slate-300 px-3 py-1.5 text-sm" />
                </div>
                <div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-500 border-b border-slate-100">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-[11px] uppercase tracking-wider">Description</th>
                <th className="px-4 py-3 font-semibold text-[11px] uppercase tracking-wider">Total Assets</th>
                <th className="px-4 py-3 font-semibold text-[11px] uppercase tracking-wider">Offer Price</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {laptops.map(l => (
                <tr key={l.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-600 text-xs">{l.offerPricePerItem ? `₹${l.offerPricePerItem}` : '-'}</td>
                  ...
                  <td className="px-4 py-3 text-right space-x-1">
                    ...
                    {deletingId === l.id ? (
                      <div className="inline-flex gap-1">
                        <button onClick={() => handleDelete(l.id)} className="px-2 py-1 bg-rose-600 text-white rounded text-xs font-bold hover:bg-rose-700">Confirm</button>
                        <button onClick={() => setDeletingId(null)} className="px-2 py-1 bg-slate-200 text-slate-600 rounded text-xs font-bold hover:bg-slate-300">No</button>
                      </div>
                    ) : (
                      <button onClick={() => setDeletingId(l.id)} className="px-2 py-1 bg-white text-rose-600 border border-slate-200 rounded text-xs font-bold hover:bg-rose-50">Del</button>
                    )}
                  </td>
                </tr>
              ))}
              {laptops.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-6 text-center text-xs text-slate-500">No inventory found.</td>
                </tr>
              )}
            </tbody>
          </table>
