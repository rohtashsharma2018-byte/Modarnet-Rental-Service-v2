...
import { collection, query, onSnapshot, doc, writeBatch, getDoc, updateDoc, increment, serverTimestamp, Timestamp } from "firebase/firestore";
...
export default function ActiveRentals() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      data.sort((a,b) => {
        const dateA = a.returnDate?.toMillis ? a.returnDate.toMillis() : (a.returnDate || 0);
        const dateB = b.returnDate?.toMillis ? b.returnDate.toMillis() : (b.returnDate || 0);
        return dateA - dateB;
      });
      setRequests(data);
  ...
  const handleReturn = async (req: RentalRequest) => {
    try {
      ...
      if (laptopDoc.exists()) {
        batch.update(laptopRef, { stock: increment(req.quantity || 1), updatedAt: serverTimestamp() });
      }
  ...
  const handleExtend = async () => {
    ...
    try {
      ...
      const currentReturnMillis = req.returnDate?.toMillis ? req.returnDate.toMillis() : (req.returnDate || Date.now());
      const newReturnDateMillis = currentReturnMillis + (extraDays * 24 * 60 * 60 * 1000);
      
      await updateDoc(doc(db, "rentalRequests", extendRequestId), {
        returnDate: Timestamp.fromMillis(newReturnDateMillis),
        duration: (req.duration || 0) + extraDays,
        totalCost: (req.totalCost || 0) + extraCost,
        status: newReturnDateMillis < Date.now() ? "overdue" : "active",
        ...
        <div className="flex-1 overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {requests.map(req => {
                const returnMillis = req.returnDate?.toMillis ? req.returnDate.toMillis() : (req.returnDate || 0);
                const isOverdue = returnMillis < Date.now();
                return (
                  <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-600">
                      <div className="text-xs">{format(returnMillis, "MMM d, yyyy")}</div>
