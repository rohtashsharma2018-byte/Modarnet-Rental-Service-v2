...
import { collection, query, where, onSnapshot, doc, writeBatch, getDoc, increment, serverTimestamp } from "firebase/firestore";
...
interface PurchaseRequestWithUser extends PurchaseRequest {
  user?: {
    name?: string;
    email?: string;
    phone?: string;
...
export default function ManagePurchases() {
  const [requests, setRequests] = useState<PurchaseRequestWithUser[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      const data: PurchaseRequestWithUser[] = [];
      const userCache = new Map<string, any>();

      for (const d of snapshot.docs) {
        const req = { id: d.id, ...d.data() } as PurchaseRequestWithUser;
  ...
  const handleApprove = async (req: PurchaseRequest) => {
    ...
    try {
      ...
      batch.update(laptopRef, { stock: increment(-req.quantity), updatedAt: serverTimestamp() });

      await batch.commit();
      toast.success("Purchase approved");
    } catch(e) {
      handleFirestoreError(e, OperationType.UPDATE, "purchaseRequests/laptops");
    }
  };
  ...
  const handleReject = async (reqId: string) => {
    try {
      ...
      handleFirestoreError(e, OperationType.UPDATE, "purchaseRequests");
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
      <div className="p-4 border-b border-slate-100 flex justify-between items-center">
        <h3 className="font-bold text-slate-800 text-sm">Recent Purchase Requests</h3>
      </div>
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 text-slate-500 border-b border-slate-100">
            <tr>
              ...
              <th className="px-4 py-3 font-semibold text-[11px] uppercase tracking-wider">Comments</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {requests.map(req => (
              <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="text-[11px] text-slate-500 truncate w-40 italic">{req.comments || 'N/A'}</div>
                </td>
                <td className="px-4 py-3">
            ...
            {requests.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-6 text-center text-xs text-slate-500">No pending purchase requests.</td>
              </tr>
            )}
          </tbody>
        </table>
