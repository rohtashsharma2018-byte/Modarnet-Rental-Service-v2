...
export default function ManageRequests() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      data.sort((a,b) => {
        const dateA = a.pickupDate?.toMillis ? a.pickupDate.toMillis() : (a.pickupDate || 0);
        const dateB = b.pickupDate?.toMillis ? b.pickupDate.toMillis() : (b.pickupDate || 0);
        return dateA - dateB;
      });
      setRequests(data);
            ...
            {requests.map(req => (
              <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-4 py-3 text-slate-600">
                  <div className="text-xs">{format(req.pickupDate?.toMillis ? req.pickupDate.toMillis() : req.pickupDate, "MMM d")} - {format(req.returnDate?.toMillis ? req.returnDate.toMillis() : req.returnDate, "MMM d")}</div>
